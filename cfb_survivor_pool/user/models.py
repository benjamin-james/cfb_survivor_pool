...
class User(UserMixin, PkModel):
    ...
    entries = relationship("Entry", backref="creator", passive_deletes=True)
    @hybrid_property
    def password(self):
        """Hashed password."""
...
class Pick(PkModel):
    ...
    __tablename__ = "pick"
    team_id = reference_col(Team.__tablename__, nullable=False, foreign_key_kwargs={"ondelete": "CASCADE"})
    team = relationship("Team")
    game_id = reference_col(Game.__tablename__, nullable=False, foreign_key_kwargs={"ondelete": "CASCADE"})
    created = db.Column(db.DateTime, nullable=False)
    game = relationship("Game")
    entry_id = reference_col(_entry_tablename, nullable=False, foreign_key_kwargs={"ondelete": "CASCADE"})
    def __init__(self, team, game, entry, created=None):
        ...
        for pick in entry.picks:
            ...
            assert sat2str(pick.game.saturday) != sat2str(game.saturday), "Picks must be on separate weeks"
    ...
    @hybrid_method
    def score(self, intra_conference_score, inter_conference_score, conference):
        ...
        if self.is_correct is not None and self.is_correct:
            if self.game.conference_game and self.game.away_team.conference == conference and self.game.home_team.conference == conference:
...
class Entry(PkModel):
    ...
    creator_id = reference_col(User.__tablename__, nullable=False, foreign_key_kwargs={"ondelete": "CASCADE"})
    ### pools = relationship("Pool", backref="entries")
    picks = relationship("Pick", backref="entry", passive_deletes=True)
    created = db.Column(db.DateTime, nullable=False)
    def __init__(self, name, creator, conference, year, created=None):
        self.name = name
